      ...
      <article className="panel-card span-full dashboard-shell">
        <header className="dashboard-topline">
          <div>
            ...
            <h2>Dashboard Analítico Nova Teens</h2>
          </div>
          ...
          {cards.map((card) => (
            <article key={card.key} className={`dashboard-kpi-card ${card.className}`}>
              ...
              <svg viewBox="0 0 100 60" className="dashboard-sparkline" aria-hidden="true">
                <defs>
                  <linearGradient id={`nova-teens-spark-${card.key}`} x1="0" y1="0" x2="0" y2="1">
        ...
        <div className="dashboard-analytics-grid">
          <article className="dashboard-analytic-panel dashboard-analytic-panel-wide">
            <header className="dashboard-panel-head">
              <div>
                <h3>Comparativo entre Encontros</h3>
                <p>Média consolidada entre 1º Nova Teens e 2º Nova Teens no recorte selecionado</p>
              </div>
              ...
              {comparativo.map((item) => (
                <div key={item.ordem} className="dashboard-bar-line">
                  <div className="dashboard-bar-line-head">
                    ...
                    <span>{formatNumber(Math.round(item.media_total))}</span>
                  </div>
                  <div className="dashboard-ranking-track">
                    <div
                      className="dashboard-ranking-fill dashboard-ranking-fill-culto"
                      style={{ width: `${clampPercent((item.media_total / maxBarValue) * 100)}%` }}
                    />
                  </div>
                </div>
              ))}
          ...
          <article className="dashboard-analytic-panel">
            <header className="dashboard-panel-head">
              <div>
                ...
                <p>{selectedDate ? 'Leitura consolidada da data filtrada' : 'Evolução de presença por data'}</p>
              </div>
              <div className="dashboard-history-pills">
                <span className="dashboard-chip">Média {formatNumber(Math.round(mediaGeral))}</span>
                <span className="dashboard-chip dashboard-chip-strong">Pico {formatNumber(pico)}</span>
              </div>
            </header>
            {historico.length > 0 ? (
              <div className="dashboard-history-track">
                {historico.slice(0, 8).map((item) => (
                  <div key={item.data_referencia} className="dashboard-history-node">
                    ...
                    <small>
                      {item.encontros
                        .map((encontro) => `${encontro.nome}: ${formatNumber(encontro.total)}`)
                        .join(' · ')}
                    </small>
                  </div>
                ))}
              ...
              <div className="dashboard-history-empty">
                <strong>Base de Nova Teens ainda vazia</strong>
            ...
            <ul className="dashboard-ranking-list">
              {comparativo.map((item, index) => (
                <li key={item.ordem}>
                  ...
                  <strong className="dashboard-ranking-value">{formatNumber(Math.round(item.media_total))}</strong>
                </li>
              ))}
            </ul>
          </article>
          ...
          <article className="dashboard-analytic-panel">
            ...
            <div className="culto-summary-grid">
              {(payload?.ultima_leitura?.encontros ?? []).map((encontro) => (
                <article key={encontro.ordem} className="inline-card">
                  <strong>{encontro.nome}</strong>
                  <span>Teens: {formatNumber(encontro.teens)}</span>
                  <span>Líderes: {formatNumber(encontro.lideres)}</span>
                  <span>Total: {formatNumber(encontro.total)}</span>
                </article>
              ))}
            </div>
          </article>
