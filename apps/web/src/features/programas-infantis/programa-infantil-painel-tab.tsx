...
export function ProgramaInfantilPainelTab({ programa, label, user }: { programa: 'um-com-deus' | 'nova-baby'; label: string; user: SessionUser }) {
            ...
            <section key={encontro.id} className="counter-card culto-counter-card">
              ...
              <div className="counter-stack">
                {([['participantes', 'Participantes'], ['lideres', 'Líderes']] as const).map(([field, fieldLabel]) => (
                  <div key={field} className="counter-row"><span>{fieldLabel}</span><div className="counter-actions"><button type="button" className="mini-button" aria-label={`Diminuir ${fieldLabel}`} onClick={() => changeCount(encontro.id, field, encontro[field] - 1)}>-</button><strong>{formatNumber(encontro[field])}</strong><button type="button" className="mini-button" aria-label={`Aumentar ${fieldLabel}`} onClick={() => changeCount(encontro.id, field, encontro[field] + 1)}>+</button></div></div>
                ))}
              </div>
